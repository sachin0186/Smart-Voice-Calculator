...
response=['Welcome to smart calculator Munna','My name is munna created by sachin', 
...
def sorry(): 
	print(response[3])
def munna():
        print(response[0]+". How can I help you?")
...
operations={'ADD':add,'PLUS':add,'SUM':add,'ADDITION':add, 
			...
			'PRODUCT':mul, 'MULTIPLY':mul,'MULTIPLICATION':mul, 'MUL':mul,
			'DIVISION':div,'MOD':mod,'REMANDER'
			:mod,'MODULAS':mod,'POWER':pow} 

# commands 
commands={'MUNNA':munna,'NAME':myname,'EXIT':end,'END':end,'CLOSE':end} 
#start of the program with welcome masg		
print('--------------'+response[0]+'------------') 
print('--------------'+response[1]+'--------------------\n') 
